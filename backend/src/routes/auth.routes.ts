...
router.get('/me', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id: userId },
      select: { id: true, email: true, name: true, plan: true, whatsappConnected: true, whatsappPhone: true, openaiApiKey: true, trialEndsAt: true, referralCode: true }
    });
    if (!user) return res.status(404).json({ error: 'Usuario no encontrado' });
    res.json({ ...user, hasApiKey: !!user.openaiApiKey, openaiApiKey: undefined });
  } catch (error) {
...
router.post('/api-key', authenticate, async (req: Request, res: Response) => {
  try {
    ...
    await prisma.user.update({ where: { id: userId }, data: { openaiApiKey: encrypted } });
